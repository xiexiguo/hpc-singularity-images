...
build_petsc()
{
    ...
    cd petsc-3.6.4
    cp ../hypre.tar.bz2 .
    export PETSC_ARCH=linux-x86_64-intel-intelmpi
    export PETSC_DIR=/mnt/petsc-3.6.4
    ./config/configure.py --download-hypre=./hypre.tar.bz2 --prefix=$PETSC --with-blas-lapack-dir=$MKLROOT/lib/intel64/ --CC=mpiicc --CXX=mpiicpc --FC=mpiifort
    make PETSC_DIR=/mnt/petsc-3.6.4/ PETSC_ARCH=linux-x86_64-intel-intelmpi all
    make PETSC_DIR=/mnt/petsc-3.6.4/ PETSC_ARCH=linux-x86_64-intel-intelmpi install
...
build_p4est()
{
    ...
    tar xf p4est-1.1.tar.gz
    cd p4est-1.1
    ./configure --enable-mpi --enable-shared --disable-vtk-binary --without-blas --prefix=$P4EST CPPFLAGS="-DSC_LOG_PRIORITY=SC_LP_ESSENTIAL" \
...
build_metis()
{
    ...
    cd metis-5.1.0
    make config prefix=$METIS cc=icc shared=1
    make -j 20 && make install
